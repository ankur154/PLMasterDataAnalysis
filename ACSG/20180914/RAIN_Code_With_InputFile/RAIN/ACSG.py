...
if __name__ == '__main__':
    ...
    def csv_from_excel(xlsxfilepath, sheetname, csvfilepath):
        ...
        try:
            ...
            your_csv_file = open(csvfilepath, 'w', newline='')
    ...
    configfilePath = "configuration.csv"
    postConfigPath = "config.txt"

    if not os.path.exists(configfilePath):
        print('configuration.csv is missing..')
        sys.exit()

    if not os.path.exists(postConfigPath):
        print('configuration.csv is missing..')
        sys.exit()
    ...
    try:
        confdata = read_configuration(configfilePath)
    except:
        pass
    try:
        postconfig = read_postConfiguration(postConfigPath)
    except:
        pass
    try:
        filtCols =  str(postconfig['FiltersAppliedOn']).split(',')
    except:
        pass
    try:
        inputfolder = str(postconfig['InputFolder'])
    except:
        pass
    try:
        outputfolder = str(postconfig['OutputFolder'])
    except:
        pass
    try:
    ...
    def extractProcess(testval, allColumnNames ):
        description = ''
        for flt in filtCols:
        ...
        if postconfig['ProductColumnName'] not in sdata:
    ...
    if not os.path.exists(inputfolder):
        ...
        for file in os.listdir(inputfolder):
            ...
            if (inputFilePath.lower().endswith('xlsx')):
                isdone = csv_from_excel(inputFilePath, xlsxSheetName, inputFilePath + ".csv")
                if (isdone):
            ...
            if isSkip is False :
                ...
                with open(outputfolder + '/' + oflname + ".csv", 'w', newline='') as csvfile:
